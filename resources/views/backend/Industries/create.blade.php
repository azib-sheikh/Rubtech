    ...
    <div class="p-3 bg-white mt-4">
        ...
        <form method="POST" action="{{ route('industries.store') }}" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input type="file" name="image" class="form-control @error('image') is-invalid @enderror" id="image" accept="image/*" onchange="previewImage(event)" required>
                @error('image')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control tinymce w-100 @error('content') is-invalid @enderror" name="content" id="content" rows="10" >{{ old('content') }}</textarea>
                @error('content')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
            ...
            <div class="mt-4">
                <button type="submit" class="btn px-4 btn-primary">Create</button>
            </div>
        </form>
